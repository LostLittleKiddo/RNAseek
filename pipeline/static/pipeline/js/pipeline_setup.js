...
(function () {
    ...
    /**
     * Render a preview table of the parsed CSV data (first 5 rows).
     */
    function renderCsvPreview() {
        ...
        var html = '<div class="csv-example">';
        html += '<div class="csv-example-header">';
        html += '<span><i class="bi bi-file-earmark-check" style="color:var(--rna-accent-green);"></i> ';
        html += 'Parsed: ' + headers.length + ' columns, ' + rows.length + ' rows</span></div>';
        html += '<table><thead><tr>';
        ...
        if (rows.length > 5) {
            html += '<tr><td colspan="' + headers.length + '" ' +
                'style="text-align:center;color:var(--rna-grey-500);font-style:italic;">' +
                '... ' + (rows.length - 5) + ' more rows</td></tr>';
        }
        html += '</tbody></table></div>';
        csvPreviewArea.innerHTML = html;
    }
        ...
        if (mode === "upload") {
            ...
            if (inputDataType === "matrix" && parsedMatrixData) {
                ...
                var allFound = parsedCsvData.rows.every(function (row) {
                    return matrixSamples.has((row[sampleCol] || "").trim());
                });
