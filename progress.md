...
| **`ModuleResult`** | 🚧 | **Model defined and migrated**, but never instantiated or queried. Tier 2 module results currently stored in `AnalysisJob.result_payload` instead of isolated `ModuleResult` rows. |
| **`ReferenceGenome`** | ❌ | **Blueprint specifies a DB model** (`genome_id`, `species`, `index_path`, `annotation_path`). Not implemented — genomes resolved via filesystem lookup in `_genome.py` using `_GENOME_FOLDER_MAP` dict. |
...
| **`ModuleResult` storage** | 🚧 | Model defined and migrated but not used; module outputs stored in `AnalysisJob.result_payload` instead |
...
| **`core_hub.html`** | ✅ | Downloads section (5 items), 5 Plotly plots (PCA, UMAP, Volcano, MA, Heatmap), 12 module cards with modals, deconvolution gateway (atlas selector, high-res toggle), 3 locked advanced spoke cards |
...
| **`core_hub.js`** (~500 lines) | ✅ | 5 Plotly rendering functions, module modal management with module-specific input builders, module submission + polling, deconvolution gateway + polling + spoke unlock, download link handlers |
...
| **`global.css`** (~500 lines) | ✅ | Complete component library: navbar, footer, buttons, cards, module cards, grids, forms, badges, modals, progress bars, responsive utilities |
...
All 11 genomes have HISAT2 indices (`.ht2`), FASTA, and GTF annotation files. Custom genome supports FASTA + GTF/GFF upload with on-demand HISAT2 index build.
...
| **2** | Data Model (ORM) | 🚧 | ~85% — `ModuleResult` unused, `ReferenceGenome` model not created |
