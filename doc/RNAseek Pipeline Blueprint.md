...
**Version:** 1.0 (Target: March 31, 2026) | **Architecture:** Multi-Tenant Asynchronous Microservices
...
*12 modular micro-pipelines that unlock after Stage 2. Triggered via `/api/modules/{name}/run`.*

| Module                   | Biological Purpose                                                 | Computational Engine    |
| ------------------------ | ------------------------------------------------------------------ | ----------------------- |
| **A. Alt Splicing**      | Detects structural transcript alterations/exon skipping.           | `IsoformSwitchAnalyzeR` |
| **B. RNA Editing**       | Base-by-base scans of BAMs to find A-to-I mutations/SNPs.          | `REDItools2`            |
| **C. Time Series**       | Clusters genes by their temporal expression across time.           | `ImpulseDE2`            |
| **D. WGCNA**             | Groups highly correlated gene networks into color modules.         | `PyWGCNA`               |
| **E. Pathways (GSEA)**   | Maps upregulated genes against KEGG/PathBank diagrams.             | `gseapy`                |
| **F. Causal Networks**   | Machine learning infers causal gene-to-gene edges.                 | `arboreto` (GRNBoost2)  |
| **G. Protein Interacts** | Fetches physical protein binding networks.                         | STRING-DB API           |
| **H. Literature NLP**    | Scans millions of PubMed abstracts for causal biology.             | INDRA Bio API           |
| **I. Survival**          | Kaplan-Meier curves predicting high/low expression survival.       | `lifelines`             |
| **J. TCGA Cancer**       | Harmonizes user data against public GDC tumor cohorts.             | `TCGAbiolinks`          |
| **K. Biomarkers**        | Flags FDA-approved diagnostic/prognostic biomarkers.               | MarkerDB API            |
| **L. MOFA / DIABLO**     | Mathematically fuses RNA-seq with ChIP-seq to find latent factors. | `mofapy2` / `mixOmics`  |
