...
| `api.py`      | 535   | 9 API views: `FileAssetDelete`, `CreateSubmissionView`, `DeleteSubmissionView`, `ChunkUploadView` (5 MB chunks, path-traversal sanitized), `CorePipelineView` (facade → validators → Celery dispatch), `JobStatusView` (stale-job detection via AsyncResult), `SessionAssetsView`, `FileDownloadView`, `ModuleRunView` (12 approved modules) |
...
| File                | Lines | Purpose                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------- | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pipeline_setup.js` | 2729  | 5-step wizard navigation, entry point/assay/library selectors, FASTQ/BAM/Matrix drop zones, chunked upload (5 MB, 3 retries, 2-min timeout), paired-end validation, CSV metadata with PapaParse, manual metadata builder, column mapping, contrast builder, per-step validation, 9 of 10 backend validation rules mirrored client-side (FASTA header `>` check is backend-only), toast notifications |
