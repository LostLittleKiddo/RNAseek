...
| `AnalysisSubmission` | Done   | UUID PK, FK to Session, `input_data_type` (fastq/alignment/matrix/tcga), `assay_type` (standard_rna/small_rna/chip_seq/methylation), `submission_name`, `library_type`, `strandedness`, `reference_genome`, `metadata_mode`, `metadata_payload` (JSON), threshold fields, `custom_genome_name`, `tcga_project_id` (TCGA cohort, e.g. `TCGA-BRCA`), `upload_dir` property |
...
| State routing (fastq/alignment/matrix/tcga)             | Done   | Full validation per entry type; conditional field requirements (matrix skips genome, TCGA skips upload/genome/metadata)                     |
...
| `GET /api/tcga/cohorts`                            | Done    | Returns 33 TCGA disease cohorts (id + name) for the submission wizard TCGA cohort selector.                                                                                       |
...
| Frontend entry point selector   | Done   | 4 radio cards (FASTQ, BAM, Matrix, TCGA); conditional UI show/hide; matrix skips genome, TCGA skips Steps 3-4  |

### Entry Point D: TCGA Disease Integration

| Feature                           | Status | Notes                                                                                                                                                                                       |
| --------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `_track_tcga.py`                  | Done   | `_route_tcga()`: 2-step pipeline (`tcga_download`, `deseq2`). Downloads STAR-Counts gene expression and clinical metadata from NCI GDC REST API, builds count matrix, auto-creates Tumor vs Normal contrast, hands off to `run_stage2_stats()` |
| GDC expression download           | Done   | `_download_tcga_expression()`: queries GDC `/files` for HTSeq STAR-Counts, downloads and combines individual sample count files into a genes × samples DataFrame                            |
| GDC clinical metadata download    | Done   | `_download_tcga_clinical()`: queries GDC `/cases` for demographic and diagnosis info, creates condition column (Tumor/Normal from `sample_type`), outputs CSV                               |
| FileAsset registration            | Done   | Registers count matrix and metadata CSV as `FileAsset` records with `is_user_uploaded=False`                                                                                                 |
| Model (`tcga_project_id`)         | Done   | `AnalysisSubmission.tcga_project_id` CharField (max 30 chars); migration `0011_tcga_disease_integration`                                                                                     |
| Backend validation                | Done   | Validates `tcga_project_id` format (`^TCGA-[A-Z]{2,6}$`); bypasses file upload and metadata validators for `tcga` input type                                                               |
| `TCGACohortsView` API             | Done   | `GET /api/tcga/cohorts` returns 33 TCGA project IDs and display names                                                                                                                       |
| Frontend wizard integration       | Done   | 4th radio card in entry point selector; cohort list panel with search filter in Step 2; wizard skips Steps 3 and 4 for TCGA; cohort selection validation; payload includes `tcga_project_id` |
| CSS (`submission_layers.css`)     | Done   | Styled cohort list items with selected state, ID badge, and scrollable container                                                                                                             |
...
| `_track_tcga.py`       | `_route_tcga()` -- TCGA pipeline driver: GDC API download (expression + clinical), FileAsset registration, auto-contrast (Tumor vs Normal), DESeq2 handoff via `run_stage2_stats()` |
...
| F   | Causal Networks | Partial-correlation + STRING PPI | Done   | `_module_networks.py`: 6-step pipeline (`net_load_data`, `net_correlation`, `net_causal_inference`, `net_string_ppi`, `net_merge_graph`, `net_serialize`). Variance-filters top genes, computes Spearman correlation + precision-matrix partial correlations, infers directed causal edges via variance-based directionality scoring with TF bonus, identifies hub genes (out-degree + betweenness centrality composite score). STRING PPI API integration with retry/backoff for rate limits. Merges causal + PPI edges into unified graph. Plotly network visualization (solid blue = causal, dashed orange = PPI). Network Statistics stat cards. Frontend form (TF textarea + STRING confidence) and result rendering complete. |
...
| I   | TCGA Cancer     | NCI GDC REST API        | Done   | Promoted to first-class **Entry Point D** (TCGA Disease Integration). `_track_tcga.py` downloads STAR-Counts + clinical from GDC, auto-creates Tumor vs Normal contrast, runs DESeq2. 33 cohorts via `/api/tcga/cohorts`. Tier 2 module card retained for future cross-cohort analysis.                                                                                                                                                                                                                                                |
...
**Summary:** 6 of 12 modules fully implemented (Alt Splicing, WGCNA, RNA Editing, Time Series, TCGA Cancer, Causal Networks). TCGA is also a first-class entry point (Entry Point D). Frontend UI complete for all 12. 6 modules have API endpoint routing but return stub payloads.
...
| `submission_layers.css` | ~2060 | Done   | 5-step wizard split-screen layout (1/3 + 2/3), step indicators with progress bar, radio cards, library-type cards, drop zones with file pills, file management panel, metadata table, CSV viewer, manual builder, column mapping, contrast builder, validation messages, threshold fields, glowing submit button, upload modal, toast animations, wizard-panel zoom (0.8), body-cloned tooltip styles, custom genome section, TCGA cohort list styles |
...
| 6     | Standard Analytical Spokes (Tier 2)                      | Partial | ~42% -- Alt Splicing, WGCNA, RNA Editing, Time Series, TCGA Cancer done (5/12). Frontend UI complete for all 12. 7 backends are stubs. |
